...
def longest_repeat(input_data):
    '''
    Find the longest repeat in a string.
    
    Parameters
    --------
    input_string: A DNA sequence (string) or trie (networkx DiGraph)
    
    Return
    --------
    A longest substring of Text that appears in Text more than once. (string)
    '''
    trie = cst.construct_suffix_trie(input_data)
    edges = trie.edges()
    repeat_substrings = []   # list of substrings that are repeated in the input_data
